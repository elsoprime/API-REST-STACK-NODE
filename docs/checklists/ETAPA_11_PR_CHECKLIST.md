...
- [ ] `npm run test:restore` en verde
- [ ] `ENABLE_MONGODB_RESTORE_DRILL=true npm run test:restore:mongodb` en verde (staging o entorno con Mongo rs0)
- [ ] `npm run go-live:check` en verde
- [ ] `/health` mantiene contrato y expone `ready` + `checks`
- [ ] readiness en `production` falla si faltan adaptadores reales de delivery

## 3. Checklist operacional

- [ ] runbook actualizado: [RUNBOOK_GO_LIVE.md](/H:/Proyectos%20FullStack/API-REST-STACK-NODE/docs/operaciones/RUNBOOK_GO_LIVE.md)
- [ ] guia local de drill Mongo disponible: [MONGO_LOCAL_RESTORE_DRILL.md](/H:/Proyectos%20FullStack/API-REST-STACK-NODE/docs/operaciones/MONGO_LOCAL_RESTORE_DRILL.md)
- [ ] evidencia de cierre actualizada: [ETAPA_11_CIERRE.md](/H:/Proyectos%20FullStack/API-REST-STACK-NODE/docs/cierres/ETAPA_11_CIERRE.md)
