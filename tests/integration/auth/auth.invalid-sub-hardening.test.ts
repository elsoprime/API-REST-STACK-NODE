...
function signInvalidAccessToken(claims: Record<string, unknown>): string {
  return jwt.sign(
    {
      ...claims,
      tokenType: 'access'
...
describe('auth invalid subject hardening', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('rejects invalid Mongo id claims before authenticated routes reach their services', async () => {
    const { app, authService, tenantService } = createInvalidClaimsHardeningApp();
    const accessToken = signInvalidAccessToken({
      sub: 'user-1',
    ...
    const requests = await Promise.all([
      ...
      request(app)
        ...
        .set('Authorization', `Bearer ${accessToken}`)
        .send({ tenantId: '507f1f77bcf86cd799439011' }),
      request(app)
        .post('/api/v1/tenant/invitations/accept')
        .set('Authorization', `Bearer ${accessToken}`)
        .send({ token: 'secure-invitation-token' })
    ]);
    ...
    expect(tenantService.switchActiveTenant).not.toHaveBeenCalled();
    expect(tenantService.acceptInvitation).not.toHaveBeenCalled();
  });
});
