describe('application startup fail-fast behavior', () => {
  beforeEach(() => {
    vi.resetModules();
  });

  it('fails fast in production when the critical database dependency does not start', async () => {
    vi.doMock('@/config/env', () => ({
      env: {
        ...
        EMAIL_RESEND_API_KEY: 're_prod_123',
    ...
    vi.doMock('@/infrastructure/database/connection', () => ({
      connectToDatabase: vi.fn().mockRejectedValue(new Error('Mongo unavailable during startup')),
    ...
    ).rejects.toThrow('Mongo unavailable during startup');
  });

  it('fails before DB bootstrap in production when delivery adapter configuration is missing even if the legacy flag is false', async () => {
